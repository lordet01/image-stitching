...
def stitch_frames(frames):
    ...
    step = 1
    batch = 5
    for i in range(step, len(frames),step*batch):
        logging.info("Pad new frame with old stitched image...")
        r_p = np.max((stitched_image.shape[1] - frames[i].shape[1], 0))
        b_p = np.max((stitched_image.shape[0] - frames[i].shape[0], 0))
        
        ####now_image = add_zero_padding(frames[i], r_p, b_p)
        ##now_image = frames[i]

        images = [stitched_image]
        for jj in range(0,batch):
            now_image = cv2.resize(frames[i+jj*step], (stitched_image.shape[1], stitched_image.shape[0]))
            images.append(now_image)
        ...
        for j, result in enumerate(results):
            ##result = cv2.resize(result, (now_image.shape[1], now_image.shape[0]))
